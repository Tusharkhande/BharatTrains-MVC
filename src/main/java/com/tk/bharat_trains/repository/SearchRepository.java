...
public interface SearchRepository extends JpaRepository<Routes, Integer>{
	@Query("SELECT new com.tk.bharat_trains.dto.response.Route(R1.trainId, R1.station, R1.arrivalTime, R1.departureTime) FROM Routes R1 INNER JOIN Routes R2 ON R1.trainId = R2.trainId WHERE R1.station = :source AND R2.station = :destination AND R1.journeyDate = :journeyDate")
